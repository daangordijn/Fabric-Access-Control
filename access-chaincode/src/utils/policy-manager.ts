...
class PolicyManager {
    ...
    public static isAuthorized(policy: AccessPolicy, manager: CertificateManager): boolean {
        switch(policy.type) {
            case 'AND':
                return policy.value.every(rule => PolicyManager.isAuthorized(rule, manager));
            case 'OR':
                return policy.value.some(rule => PolicyManager.isAuthorized(rule, manager));
            case 'NOT':
                return !PolicyManager.isAuthorized(policy.value, manager);
            case 'EQUALS':
                return manager.assertAttributeEquals(policy.name, policy.value);
            case 'INCLUDES':
                return manager.assertAttributeIncludes(policy.name, policy.value);
        }
    }
}
